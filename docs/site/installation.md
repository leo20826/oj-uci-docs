...
Next, we will set up the database using MariaDB. The K23OJ is only tested to work with MySQL, and it is unlikely to work with anything else. Please visit [the MariaDB site](https://mariadb.org/download/?t=repo-config) and follow the download instructions.
...
You should see `(venv)` prepended to your shell. Henceforth, `(venv)` commands assume you are in the code directory, with the virtual environment active.
...
```shell-session
(venv) $ git clone --recursive https://github.com/huythedev/K23OJ.git site
(venv) $ cd site
```
...
You will now need to configure `dmoj/local_settings.py`. You should make a copy of [this sample settings file](https://github.com/huythedev/k23oj-docs/blob/master/sample_files/local_settings.py) and read through it, making changes as necessary. Most importantly, you will want to update MariaDB credentials.
...
K23OJ uses `sass` and `autoprefixer` to generate the site stylesheets. K23OJ comes with a `make_style.sh` script that may be run to compile and optimize the stylesheets.
...
(venv) $ ./manage.py compilejsi18n
```

## Setting up Celery

The K23OJ uses Celery workers to perform most of its heavy lifting, such as batch rescoring submissions. We will use Redis as its broker, though note that other brokers that Celery supports will work as well.
...
```shell-session
(venv) $ ./manage.py loaddata navbar
(venv) $ ./manage.py loaddata language_small
(venv) $ ./manage.py loaddata demo
```
...
First, copy our `uwsgi.ini` ([link](https://github.com/huythedev/k23oj-docs/blob/master/sample_files/uwsgi.ini)). You should change the paths to reflect your install.
...
Copy our `site.conf` ([link](https://github.com/huythedev/k23oj-docs/blob/master/sample_files/site.conf)) to `/etc/supervisor/conf.d/site.conf`, `bridged.conf` ([link](https://github.com/huythedev/k23oj-docs/blob/master/sample_files/bridged.conf)) to `/etc/supervisor/conf.d/bridged.conf`, `celery.conf` ([link](https://github.com/huythedev/k23oj-docs/blob/master/sample_files/celery.conf)) to `/etc/supervisor/conf.d/celery.conf` and fill in the fields.
...
You should copy the sample `nginx.conf` ([link](https://github.com/huythedev/k23oj-docs/blob/master/sample_files/nginx.conf)), edit it and place it in wherever it is supposed to be for your nginx install.
...
Now copy `wsevent.conf` ([link](https://github.com/huythedev/k23oj-docs/blob/master/sample_files/wsevent.conf)) to `/etc/supervisor/conf.d/wsevent.conf`, changing paths, and then update supervisor and nginx.
